...
const Register: React.FC = () => {
  ...
  const handleRegister = () => {
    ...
    console.log('Register', { firstName, email, password, confirmPassword, dob });
  };
...
<form className="w-full px-1 py-2 mb-3">
        <InputField label="First Name" type="name" value={firstName} onChange={(e) => setFirstName(e.target.value)} />
        <InputField label="Email Address:" type="email" value={email} onChange={(e) => setEmail(e.target.value)} />
        <InputField label="Password:" type="password" value={password} onChange={(e) => setPassword(e.target.value)} />
        <InputField label="Confirm Password:" type="confirmpassword" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} />
        <InputField label="Date of birth:" type="dob" value={dob} onChange={(e) => setDob(e.target.value)} />
</form>
...
<div className="flex items-center gap-3 mb-9">
      ...
      <div className="text-base font-bold text-[#6f6d6d82] mb-12">
        Already a User?        
        <Link to="/login" className="text-base font-bold text-[#6f6d6d82] underline">LOGIN HERE</Link>
